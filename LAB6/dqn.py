...
class Net(nn.Module):
    def __init__(self, state_dim=8, action_dim=4, h_dim=(400, 300)):
        super().__init__()
        ## TODO ##
        self.net = nn.Sequential(
            nn.Linear(state_dim, h_dim[0]),
            nn.ReLU(),
            nn.Linear(h_dim[0], h_dim[1]),
            nn.ReLU(),
            nn.Linear(h_dim[1], action_dim)
        )
...
class DQN:
    ...
    def update(self, sum_steps):
        if sum_steps % self.freq == 0:
            self._update_behavior_network(self.gamma)
        if sum_steps % self.target_freq == 0:
...
def train(args, env, agent, writer):
    ...
    for episode in range(args.episode):
        ...
        for t in itertools.count(start=1):  # play an episode
            ...
            # update
            if sum_steps >= args.warmup:
                agent.update(sum_steps)

            state = next_state
            sum_reward += reward
            sum_steps += 1
            if done:
                ewma_reward = 0.05 * sum_reward + (1 - 0.05) * ewma_reward
                writer.add_scalar('Train/Episode Reward', sum_reward, sum_steps)
                writer.add_scalar('Train/Ewma Reward', ewma_reward, sum_steps)
                print(f'Step: {sum_steps}\tEpisode: {episode}\tLength: {t:3d}\tTotal reward: {sum_reward:.2f}\tEwma reward: {ewma_reward:.2f}\tEpsilon: {epsilon:.3f}')
                break
    env.close()
...
def test(args, env, agent, writer):
    ...
    for n_episode, seed in enumerate(seeds):
        ...
        for t in itertools.count(start=1):  # play an episode
            ...
            if done:
                writer.add_scalar('Test/Episode Reward', sum_reward, n_episode)
                print(f'total reward: {sum_reward:.2f}')
